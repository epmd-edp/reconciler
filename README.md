...
Reconciler is en EDP operator responsible for work with EDP tenant database.
...
```bash
helm install reconciler --namespace <edp_cicd_project> --set name=reconciler --set namespace=<edp_cicd_project> --set platform=<platform_type> --set image.name=epamedp/jenkins-operator --set image.version=<operator_version> deploy-templates
```
