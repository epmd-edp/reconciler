...
import (
	...
	rs "github.com/epmd-edp/reconciler/v2/pkg/repository/stage"
...
func prevStageAdded(tx sql.Tx, stage stage.Stage) bool {
	...
	stageId, err := rs.GetStageIdByPipelineNameAndOrder(tx, stage.Tenant, stage.CdPipelineName, stage.Order-1)
	if err != nil {
...
func updateStageStatus(tx sql.Tx, id *int, stage stage.Stage) error {
	...
	err := rs.UpdateStageStatus(tx, stage.Tenant, *id, stage.Status)
	if err != nil {
...
func getStageIdOrCreate(tx sql.Tx, edpRestClient *rest.RESTClient, stage stage.Stage) (*int, error) {
	id, err := rs.GetStageId(tx, stage.Tenant, stage.Name, stage.CdPipelineName)
	if err != nil {
		return nil, err
	}
	if id != nil {
...
func createStage(tx sql.Tx, edpRestClient *rest.RESTClient, stage stage.Stage) (*int, error) {
	...
	id, err := rs.CreateStage(tx, stage, cdPipeline.Id)
	if err != nil {
...
func insertAutotestQualityGate(tx sql.Tx, cdStageId int, gate stage.QualityGate, schemaName string) error {
	entityIdsDTO, err := rs.GetCodebaseAndBranchIds(tx, *gate.AutotestName, *gate.BranchName, schemaName)
	if err != nil {
		return err
	}

	_, err = rs.CreateQualityGate(tx, gate.QualityGate, gate.JenkinsStepName, cdStageId, &entityIdsDTO.CodebaseId, &entityIdsDTO.BranchId, schemaName)

	return err
}

func insertManualQualityGate(tx sql.Tx, cdStageId int, gate stage.QualityGate, schemaName string) error {
	_, err := rs.CreateQualityGate(tx, gate.QualityGate, gate.JenkinsStepName, cdStageId, nil, nil, schemaName)
