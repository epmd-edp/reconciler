...
func (s CodebaseBranchService) PutCodebaseBranch(codebaseBranch codebasebranch.CodebaseBranch) error {
	...
	txn, err := s.DB.Begin()
	if err != nil {
		return errWrap.Wrap(err, "an error has occurred while opening transaction")
	}
	...
	if err != nil {
		_ = txn.Rollback()
		return errWrap.Wrapf(err, "an error has occurred while getting Codebase Branch id or create",
	...
	if err != nil {
		_ = txn.Rollback()
		return errWrap.Wrapf(err, "an error has occurred during status creation", "name", codebaseBranch.Name)
	}
	...
	if err != nil {
		_ = txn.Rollback()
		return errWrap.Wrapf(err, "an error has occurred during retrieving codebase id", "id", cbId)
	}

	if err := repository.CreateCodebaseAction(*txn, *cbId, *actionLogId, schemaName); err != nil {
		_ = txn.Rollback()
		return errWrap.Wrap(err, "an error has occurred during codebase_branch_action")
	}
	log.V(2).Info("codebase_action has been updated")

	if err := codebase_branch.UpdateStatusByCodebaseBranchId(*txn, *id, codebaseBranch.Status, codebaseBranch.Tenant); err != nil {
		_ = txn.Rollback()
		return errWrap.Wrapf(err, "an error has occurred during the update of codebase branch %v", codebaseBranch.Name)
	}
...
func createCodebaseBranch(txn sql.Tx, codebaseBranch codebasebranch.CodebaseBranch, schemaName string) (*int, error) {
	...
	id, err := codebase_branch.CreateCodebaseBranch(txn, codebaseBranch.Name, *beId,
...
func getCodebaseBranchIdOrCreate(txn sql.Tx, codebaseBranch codebasebranch.CodebaseBranch, schemaName string) (*int, error) {
	...
	id, err := codebase_branch.GetCodebaseBranchId(txn, codebaseBranch.AppName, codebaseBranch.Name, schemaName)
	if err != nil {
		return nil, err
	}
	if id == nil {
...
func updateCodebaseBranch(txn sql.Tx, codebaseBranch codebasebranch.CodebaseBranch, id int, schemaName string) error {
	...
	err := codebase_branch.UpdateCodebaseBranch(txn, id, codebaseBranch.Version, codebaseBranch.BuildNumber,
