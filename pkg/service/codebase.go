...
import (
	...
	jenkins_slave "github.com/epmd-edp/reconciler/v2/pkg/repository/jenkins-slave"
...
func createBE(txn *sql.Tx, c codebase.Codebase, schemaName string, scope string) (*int, error) {
	...
	if c.JobProvisioning != "" {
		jpId, err := jp.SelectJobProvision(*txn, c.JobProvisioning, scope, schemaName)
		if err != nil || jpId == nil {
